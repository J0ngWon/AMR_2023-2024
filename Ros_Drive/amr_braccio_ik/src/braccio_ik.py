...
import time
import numpy as np
from std_msgs.msg import Float64MultiArray, Int32  # Int32 메시지 추가
    ...
    def calculate_thetas(self, x, y, z, beta, gamma):
        ...
        try:
            ...
            if x < 0.34:
                ...
                theta2 = rho + psi - np.radians(95)

            theta3 = np.pi - phi

            # Step 5: 조건에 따라 theta4 계산
            if x < 0.34:
                theta4 = (theta2 + theta3) - np.radians(65)  
            else:
...
class BraccioObjectTargetInterface:
    ...
    def __init__(self):
        ...
        self.end_pub = rospy.Publisher('/braccioend', Int32, queue_size=20)  # braccioend 토픽 발행
        ...
        self.last_coordinates = None  # 마지막 좌표를 저장하기 위한 변수
    ...
    def bk_data_callback(self, data):
        ...
        if len(data.data) == 3:
            ...
            # 새로운 좌표가 기존 좌표와 다른지 확인
            if self.last_coordinates is None or (x != self.last_coordinates[0] or y != self.last_coordinates[1] or z != self.last_coordinates[2]):
                rospy.loginfo(f"bk_data에서 업데이트된 목표 위치 - x: {x}, y: {y}, z: {z}")

                # 마지막 좌표 업데이트
                self.last_coordinates = (x, y, z)

                # 목표 위치를 PoseStamped로 설정
                self.target_position = PoseStamped()
    ...
    def move_robot_to_final_pose(self):
        ...
        try:
            ...
            # braccioend 토픽으로 1 발행
            self.end_pub.publish(1)
            rospy.sleep(1)
            
            # 발행 후 로그 추가
            rospy.loginfo("로봇 동작 완료. braccioend 토픽으로 1 발행 완료.")
